...
for x in range(len(datarows)):
    ...
    if os.path.exists(image_path):
        ...
        label = datarows[x][3]
        print(len(text_rows))
        print(len(text_rows[num]))
        if label == str(0) and real_num < 300:
            label = "real"
            real_num = real_num + 1
            num = num + 1
        elif label == str(1) and fake_num < 300:
            label = "fake"
            fake_num = fake_num + 1
            num = num + 1
        if label == str(0) and real_num >= 300:
            continue
        if label == str(1) and fake_num >= 300:
            continue      
        ...
        for a in range(len(text_kge)):
            text_knowledge = text_kge[a]
        ...
        final_text_knowledge = text_kge[text_max_index]['body']
        print(f"源文本:{text}")
        print(f"文本知识:{final_text_knowledge}")
        image_max_index = 0
        image_max_similarity = 0
        for a in range(len(image_kge)):
            image_knowledge = image_kge[a]
        ...
        final_image_knowledge = image_kge[image_max_index]['body']
...
with open("train_MR2/MR2_real_logits0.8013.txt",'a',encoding='utf-8') as file:
    # real_logits = file.read()
    file.write(str(real_logits))
with open("train_MR2/MR2_fake_logits0.8013.txt",'a',encoding='utf-8') as file:
    # fake_logits = file.read()
    file.write(str(fake_logits))
with open("train_MR2/MR2_labels0.8013.txt",'a',encoding='utf-8') as file:
    # labels = file.read()
