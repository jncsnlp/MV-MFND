...
#fakeddit
with open("/home/jncsnlp4/tb/Qwen2-VL-main/train_MR2/train_data.csv",'r',encoding='utf-8') as file:
    csv_dict_reader = csv.reader(file)
    datarows = list(csv_dict_reader)


with open("/home/jncsnlp4/tb/Qwen2-VL-main/train_MR2/text_knowledge_train_MR2-en2.csv",mode='r',encoding="utf-8")as file:
    csv_dict_reader = csv.reader(file)
    text_rows = list(csv_dict_reader)


with open('/home/jncsnlp4/tb/Qwen2-VL-main/train_MR2/image_knowledge_train_MR2-en2.csv',mode='r',encoding="utf-8")as file:
    csv_dict_reader = csv.reader(file)
    image_rows = list(csv_dict_reader)


with open('/home/jncsnlp4/tb/prompt-llava/MRtrain/MR_text_max_index.txt','r',encoding='utf-8') as file:
    text_indexs = file.read()

with open('/home/jncsnlp4/tb/prompt-llava/MRtrain/MRtwitter_image_max_index.txt','r',encoding='utf-8') as file:
    image_indexs = file.read()

with open('/home/jncsnlp4/tb/Qwen2-VL-main/train_MR2/text_question.txt','r',encoding = 'utf-8') as file:
    text_questions = file.readlines()

with open('/home/jncsnlp4/tb/Qwen2-VL-main/train_MR2/image_question.txt','r',encoding='utf-8') as file:
...
for x in range(len(datarows)):
    ...
    if os.path.exists(image_path):
        ...
        text_kge = ast.literal_eval(text_rows[num][1])
        image_kge = ast.literal_eval(image_rows[num][1])
        
        final_text_knowledge = text_kge[text_index]['body']
        print(f"源文本:{text}")
        print(f"文本知识:{final_text_knowledge}")
        final_image_knowledge = image_kge[image_index]['body']
        print(f"图像知识:{final_image_knowledge}")
        # if max(text_real_prob,text_fake_prob)<0.6 or text_max_similarity > 1.6 :
        text_output = ""
        image_output = ""
        mm_output = ""
        if max(text_real_prob,text_fake_prob)<0.9490 :    
        ...
        if max(image_real_prob,image_fake_prob)<0.9490:
        ...
        if max(mm_real_logits,mm_fake_logits) < 0.9490 :
...
with open("MRtrain/MR_real_logits0.9490.txt",'a',encoding='utf-8') as file:
    # real_logits = file.read()
    file.write(str(real_logits))
with open("MRtrain/MR_fake_logits0.9490.txt",'a',encoding='utf-8') as file:
    # fake_logits = file.read()
    file.write(str(fake_logits))
with open("MRtrain/MR_labels0.9490.txt",'a',encoding='utf-8') as file:
    # labels = file.read()
