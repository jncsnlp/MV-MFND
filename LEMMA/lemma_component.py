...
from utils import offlineImg_process, gpt_no_image
...
class LemmaComponent:
    ...
    def __call__(self, *args, **kwargs):
        ...
        for i in range(self.max_retry):
            try:
                if self.model == 'Qwen-2-vl':
                    print(f"online_image:{self.online_image}")
                    if self.online_image:
                        ...
                        result = offlineImg_process(prompt=prompt, image_path=image_path,
                                                    max_tokens=self.max_tokens, temperature=self.temperature)
                elif self.model == 'Qwen-2-t':
                    result = gpt_no_image(prompt, model='Qwen-2-t', max_tokens=self.max_tokens,
